...
/**
 * Game2048GUI class manages the displaying of the <b>logo</b>, <b>score</b> and
 * the <b>grid</b>. It does not contain any logic of the game.
 */
class Game2048GUI {
  /**
   ...
   */
  private final LineBorder OLD_SLOT_SETTINGS = new LineBorder(Color.red, BORDER_THICKNESS, false);
  ...
  /**
   * An array that stores all the filename of the <i>slots</i> filename. Index
   * 0 contains the filename of the image for value 2, index 1 contains the
   * image for value 4 and etc.
   */
  ...
  /**
   * The grid that wraps around the slots. Changing each individual JLabel
   * will alter the look of the grid.
   */
  ...
  // ================= PRIVATE METHODS ====================//
  /**
   * Initialize the GUI with the config file <code>gui_config.txt</code>. If
   * the file does not exist, meit will throw an IOException error.
   */
  private void initConfig() {
    /*
     * TO DO: initialize the following variables with information read from
     * the config file - logoIconFile - slotIconFile
     */
    try {
      BufferedReader read = new BufferedReader(new FileReader("gui_config.txt"));
      logoIconFile = read.readLine();
      int numOfIcons = Integer.parseInt(read.readLine());
      slotIconFile = new String[numOfIcons];
      for (int i = 0; i < numOfIcons; i++) {
        slotIconFile[i] = read.readLine();
      }
      read.close();
  ...
  /**
   * Create a <b>Logo</b> <i>JPanel</i> to be used in a <i>JFrame</i> or
   * <i>JPanel</i>
   */
  ...
  /**
   * Create a <b>Score</b> <i>JPanel</i> to be used in a <i>JFrame</i> or
   * <i>JPanel</i>
   */
  ...
  /**
   * Create a <b>Grid</b> <i>JPanel</i> to be used in a <i>JFrame</i> or
   * <i>JPanel</i>.
   * <p>
   * The <b>grid</b> is the interface that contains the individual
   * <b>slots</b>.
   */
   ...
   * <ul>
   * <li>mainPanel - The <b>main</b> JPanel that wraps around all the other
   * sub JPanels
  ...
  private void createFrame() {
    ...
    // Create the panel for the score
   ...
   * 
   * @param time
   *            Time delaying represented in milliseconds. <i>(Example: 1000 =
   *            1 second)</i>
   */
   ...
   */
  public void showGameOver(long highScore, long currentScore) {
    try {
      if (currentScore>highScore){
        score = ("Your new high score is: "+currentScore);
        highScore = currentScore;
        
        BufferedWriter writeScore = new BufferedWriter (new FileWriter ("high_score.txt"));
        writeScore.write(Long.toString(highScore));
        writeScore.close();
      } else {
        score = ("Your score is: "+currentScore);
      }
      JOptionPane.showMessageDialog(null, ("Game Over!"+"\n"+score), "The Game Is Finished!", JOptionPane.PLAIN_MESSAGE);
      System.exit(0);
    } catch(IOException  e) {
   ...
   */
  public void showGameWon(long highScore, long currentScore) {
    try{
      if (currentScore>highScore){
        score = ("Your new high score is: "+currentScore);
        highScore = currentScore;
  ...
  /**
   * Add <i>Listener</i> to the <b>mainFrame</b> JFrame to capture user's
   * input.
   *
   * @param listener
   *            The listener to add to <i>mainFrame</i>
   */
   ...
   *
   * @param rowIndex
   *            The row index on the grid represented by the <b>slots</b>
   *            array.
   * @param columnIndex
   *            The column index on the grid represented by the <b>slots</b>
   *            array.
   * @param slotIndex
   *            The index of the array that contains the <b>slotIconFile</b>
   *            filename.
   */
  public  void setNewSlotBySlotIndex(int rowIndex, int columnIndex, int slotIndex) {
    ...
    guiGrid[rowIndex][columnIndex].setBorder(OLD_SLOT_SETTINGS);
  }
  public  void setNewSlotBySlotNewIndex(int rowIndex, int columnIndex, int slotIndex) {
   ...
   *            array.
   * @param slotValue
   *            The value of the new slot. Value must be one of the values in
   *            the game.
   */
  ...
  /**
   * It takes in a grid array with slots represented by indexes and displays
   * it.
   *
   * @param grid
   *            The grid that contains all the slots with indexes of the
  ...
  /**
   * It takes in a grid array with slots represented by values and displays
   * it.
   *
   * @param grid
   *            The grid that contains all the slots with values of the
   *            current state of the game.
   */
  ...
  /**
   * Main of the program. It starts by initializing Game2048Gui, Game2048 and
   * Game2048Listener object.
   */
